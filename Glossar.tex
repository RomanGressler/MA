...
\newacronym[description={A by a computer generated environment. The user sees through a head mounted display and can interact in the Virtual Reality with some kind of controlers.}]{vr}{VR}{Virtual Reality}
...
\newacronym[description={A file format for graphs. Is used for example in \gls{see} to import and export \glspl{city}.}]{glx}{GLX}{ Graph eXchange Language}
...
\newglossaryentry{utest}{name={Mann-Whitney-U-Test},description={A test that compares two indepentend samples with ordinal scales to find out whether there is a difference between the two groups. The Mann-Whitney-U-Test does not require a normal distribution. }}
...
\newglossaryentry{android}{name={Android},description={An operating system for portable device such as smartphones and tablets.}}
...
\newglossaryentry{gameObject}{name={GameObject},description={A fundamental object in a \gls{unity} \glspl{scene}.}}
\newglossaryentry{scene}{name={scene},description={A collection of components that form overall picture. A scene could contain for example environmental or UI components.}plural=scences}
